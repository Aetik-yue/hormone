...
在“设置 → 自定义节次时间”中，默认开启“后续节次同步顺延”。只需修改第一节的开始时间，后续已设置的节次就会按相同分钟数前移或后移，保留课长、课间和午休间隔。例如第一节从 08:00 调整到 08:30，第二节 08:55 会变为 09:25。关闭开关可单独修改第一节；其他节次始终支持单独编辑。未设置的节次保持留空，跨天调整会提示并保留原配置。
